...
{
  auto op_func = [](hostrpc::port_t, hostrpc::page_t *page) {
  ...
  auto cl_func = [](hostrpc::port_t, hostrpc::page_t *page) {
    for (unsigned c = 0; c < 64; c++)
      {
        hostrpc::cacheline_t &line = page->cacheline[c];
        for (unsigned i = 0; i < 8; i++)
...
uint32_t stress_pool_client::run(uint32_t state)
{
  ...
  auto fill = [&](hostrpc::port_t, hostrpc::page_t *page) {
    __builtin_memcpy(page, &scratch, sizeof(hostrpc::page_t));
  };
  auto use = [&](hostrpc::port_t, hostrpc::page_t *page) {
